...
class Encoder:
    ...
    def encode(self, text_list, dataset_name):
        ...
        nq_distilbert_embedding = self.load_embedding(nq_distilbert_dir)

        use_embedding = self.normalize(use_embedding)
        sbert_embedding = self.normalize(sbert_embedding)
        nq_distilbert_embedding = self.normalize(nq_distilbert_embedding)

        embedding = np.concatenate([use_embedding, sbert_embedding, nq_distilbert_embedding], axis=1)
